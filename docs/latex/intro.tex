...
search space, which is the basis of the security discussion in
...
The assumed adversary is a passive or active attacker on the network path between
client and server who can capture, replay or modify frames but does not control
either endpoint. Against this adversary the prototype is expected to protect
confidentiality; it makes no claim about integrity or availability.

\section{Identified Weaknesses}
\begin{enumerate}
  \item \textbf{Trivial key spaces.} Caesar has 25 useful keys and S-DES has
        $2^{10}=1{,}024$; both are exhaustively searchable in microseconds. A
        brute-force script that decrypts one captured block under every candidate
        key recovers the plaintext immediately.
  \item \textbf{ECB mode.} S-DES is applied byte-by-byte with no chaining and no
        initialisation vector, so identical plaintext bytes always map to identical
        ciphertext bytes. Frequency analysis over an 8-bit block size therefore
        leaks the structure of the payload.
  \item \textbf{Key transported with the ciphertext.} The JSON header contains the
        key in cleartext, which defeats the encryption entirely for any observer of
        the channel. This is the single most serious defect.
  \item \textbf{No integrity or authentication.} Without a MAC or digital
        signature, an attacker can flip ciphertext bytes and the receiver will
        accept the corrupted plaintext; there is also no way to tell whether the
        peer is the legitimate server.
  \item \textbf{No replay protection.} Frames carry no nonce, sequence number or
        timestamp, so a captured frame can be replayed indefinitely.
  \item \textbf{Statistical weakness of classical ciphers.} Caesar preserves
        letter frequency, and Playfair preserves digraph frequency, so both fall to
        classical cryptanalysis on modest amounts of ciphertext.
\end{enumerate}

\section{Recommended Hardening}
Table~\ref{tab:harden} maps each weakness to the accepted modern remedy.

\begin{table}[!ht]
\centering
\caption{Weaknesses mapped to recommended mitigations}
\label{tab:harden}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|l|l|}
\hline
\textbf{Weakness} & \textbf{Recommended mitigation} \\ \hline
Small key space & AES-128/256 or another cipher with a 128-bit or larger key \\ \hline
ECB mode & Authenticated mode such as AES-GCM or ChaCha20-Poly1305 with a random nonce \\ \hline
Key in header & Diffie--Hellman key agreement, or run the protocol inside TLS \\ \hline
No integrity & HMAC-SHA256 over header and ciphertext, verified before decryption \\ \hline
No replay protection & Monotonic sequence numbers plus per-session nonces \\ \hline
Weak passphrase keys & Key derivation with PBKDF2, scrypt or Argon2 and a random salt \\ \hline
Single-threaded server & Thread or asyncio per connection, with per-connection limits \\ \hline
\end{tabular}%
}
\end{table}

\section{Mapping to the CIA Triad}
\begin{table}[!ht]
\centering
\caption{Security properties achieved by the prototype}
\label{tab:cia}
\begin{tabular}{|l|l|p{6.5cm}|}
\hline
\textbf{Property} & \textbf{Status} & \textbf{Reason} \\ \hline
Confidentiality & Partial & Obfuscation only; key spaces are searchable and the key is sent in the clear \\ \hline
Integrity & Not provided & No MAC, checksum or signature over the frame \\ \hline
Availability & Not addressed & Single-threaded server, no rate limiting or timeouts \\ \hline
Authentication & Not provided & Neither endpoint proves its identity \\ \hline
Non-repudiation & Not provided & No signatures or audit log of exchanges \\ \hline
\end{tabular}
\end{table}
...
\section{Conclusion}
The prototype meets every functional objective. Three ciphers of increasing
complexity were implemented in a single reusable module, wrapped in a documented
application-layer frame, and exercised across a TCP connection in both message and
bulk-file modes. Correctness was demonstrated rather than assumed: the 1~MB file
decrypted on the server and the 10~KB file decrypted on the client were both
byte-identical to their sources. The measured S-DES throughput of about
0.13~MB/s quantifies the price of a purely educational implementation. Equally
important, the exercise made the limits of the design explicit: obfuscation is
not confidentiality, and a cipher without key management, integrity protection and
a sound mode of operation provides no meaningful defence.

\section{Future Work}
\begin{enumerate}
  \item Replace S-DES with AES-GCM from a vetted library and remove the key from
        the header in favour of an ephemeral Diffie--Hellman exchange.
  \item Add HMAC-SHA256 verification and per-frame sequence numbers.
  \item Complete the S-DES message-mode reply path on the client.
  \item Stream large files in fixed-size chunks instead of buffering an entire
        1~MB payload in memory.
  \item Serve connections concurrently and add read timeouts and size limits.
  \item Add an automated test suite that asserts round-trip equality and rejects
        malformed headers and keys.
\end{enumerate}

\chapter*{References}
\addcontentsline{toc}{chapter}{References}
\begin{enumerate}
  \item Project source repository, \emph{IAS client--server cipher prototype}.
        \url{https://github.com/Trinadh-31/IAS}
  \item W. Stallings, \emph{Cryptography and Network Security: Principles and
        Practice}, Appendix on Simplified DES. \url{https://williamstallings.com/Cryptography/}
  \item National Institute of Standards and Technology, \emph{FIPS 197: Advanced
        Encryption Standard (AES)}. \url{https://csrc.nist.gov/pubs/fips/197/final}
  \item National Institute of Standards and Technology, \emph{SP 800-38D:
        Recommendation for Block Cipher Modes of Operation: Galois/Counter Mode
        (GCM) and GMAC}. \url{https://csrc.nist.gov/pubs/sp/800/38/d/final}
  \item Python Software Foundation, \emph{socket --- Low-level networking
        interface}. \url{https://docs.python.org/3/library/socket.html}
  \item Python Software Foundation, \emph{json --- JSON encoder and decoder}.
        \url{https://docs.python.org/3/library/json.html}
  \item E. Rescorla, \emph{RFC 8446: The Transport Layer Security (TLS) Protocol
        Version 1.3}. \url{https://www.rfc-editor.org/rfc/rfc8446}
\end{enumerate}
