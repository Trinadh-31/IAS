...
\begin{enumerate}
  ...
  \item Record the residual weaknesses and known defects observed while testing
        the prototype.
\end{enumerate}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.98\linewidth]{images/shot_caesar.png}
\caption{Execution output for the Caesar cipher: the client encrypts \texttt{Information Assurance and Security} with shift~3 and decrypts the reply, while the server prints the ciphertext it received and the recovered plaintext}
\label{fig:run:caesar}
\end{figure}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.98\linewidth]{images/shot_playfair.png}
\caption{Execution output for the Playfair cipher with keyword \texttt{SECURE}: \texttt{HELLO WORLD} travels as \texttt{ISKYIQEWUMKC} and is recovered on the server as \texttt{HELXLOWORLDX}, showing the filler letters the digraph preparation inserts}
\label{fig:run:playfair}
\end{figure}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.98\linewidth]{images/shot_sdes_msg.png}
\caption{Execution output for an S-DES message: \texttt{HELLO} encrypts to the five ciphertext bytes \texttt{e050a5a547} and the server recovers \texttt{b'HELLO'}. The client only reports that a file came back, which is the known gap in the S-DES reply path}
\label{fig:run:sdesmsg}
\end{figure}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.98\linewidth]{images/shot_sdes_file.png}
\caption{Execution output for the 1~MB S-DES file transfer: the client sends 1{,}048{,}576 ciphertext bytes, the server stores and decrypts them, and the 10~KB reply file is decrypted back on the client}
\label{fig:run:sdesfile}
\end{figure}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\linewidth]{images/shot_makefiles.png}
\caption{Execution output of \texttt{make\_files.py}: the two sample payloads are created with the exact sizes used in every test}
\label{fig:run:makefiles}
\end{figure}
...
Every case in Table~\ref{tab:tests} was captured directly from the two terminals
while it ran. The message-mode runs are shown in Figure~\ref{fig:run:caesar}
(Caesar), Figure~\ref{fig:run:playfair} (Playfair) and
Figure~\ref{fig:run:sdesmsg} (S-DES); the bulk transfer is shown in
Figure~\ref{fig:run:sdesfile} and the generation of the sample payloads in
Figure~\ref{fig:run:makefiles}. In each screenshot the upper window is the
client and the lower window is the server, so the ciphertext printed by the
sender can be compared with the ciphertext and plaintext printed by the receiver.

\section{Error Handling}
The client validates nothing itself and relies on the cipher module to reject bad
key material. Supplying a three-bit S-DES key therefore raises a
\texttt{ValueError} from \texttt{sdes\_encrypt\_bytes()} and terminates the
client, as captured in Figure~\ref{fig:run:invalidkey}. The error is correct but
unhandled: a production client would catch it and re-prompt.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.98\linewidth]{images/shot_invalid_key.png}
\caption{Execution output for an invalid S-DES key: a three-bit key is rejected by \texttt{sdes\_encrypt\_bytes()} with \texttt{ValueError: SDES key must be 10 bits}}
\label{fig:run:invalidkey}
\end{figure}
...
1~MB (1{,}048{,}576 B) & 7.50~s & 7.52~s & $\approx$ 0.13~MB/s \\ \hline
10~KB (10{,}240 B) & 0.070~s & 0.070~s & $\approx$ 0.13~MB/s \\ \hline
